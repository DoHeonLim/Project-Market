 ...
 * Description : Supabase 채팅방 실시간 구독 로직 (타입 적용)
 * Author : 임도헌
 *
 * History
...
interface SubscribeOptions {
  ...
  onMessageRead: (payload: MessageReadPayload) => void; // 읽음 처리 수신 콜백
}
...
export function subscribeToRoomUpdates({
  ...
  const channels = roomIds.map((roomId) => {
    ...
    channel.on("broadcast", { event: "message_read" }, ({ payload }) => {
      const readInfo = payload as MessageReadPayload;

      // 내 읽음 처리만 반영 (현재 room에 해당할 경우만)
      if (readInfo.userId === userId && readInfo.roomId === roomId) {
        onMessageRead(readInfo);
      }
    });
